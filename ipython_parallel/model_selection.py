...
class RandomizedGridSeach(object):
    ...
    def map_tasks(self, f, skip_aborted=True):
        if skip_aborted:
            return [f(task) for task_group in self.task_groups
                    for task in task_group if not is_aborted(task)]
        else:
            return [f(task) for task_group in self.task_groups
        ...
        for params in self.all_parameters:
            ...
            for cv_split_filename in cv_split_filenames:
                task = self.lb_view.apply(
                    compute_evaluation, model, cv_split_filename,
                    params=params)
    ...
    def launch_for_arrays(self, model, parameter_grid, X, y, n_cv_iter=5,
        ...
        return self.launch_for_splits(model, parameter_grid,
                                      cv_split_filenames, pre_warm=pre_warm,
    ...
    def boxplot_parameters(self, display_train=False):
        ...
        pl.figure()
        for i, (param_name, param_values) in enumerate(
